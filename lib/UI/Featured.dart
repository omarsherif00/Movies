    ...
    Widget BuildSlider(double height, double width,List<PopularResults> results) {
      return CarouselSlider.builder(
        ...
        itemBuilder: (BuildContext context, int itemIndex, int pageViewIndex) {
          ...
          final isAdded = listProvider.isMovieInWatchlist("${popularResults.id}");
          return InkWell(
            onTap: () {
                            ...
                            onTap: () async {
                              if (isAdded) {
                                await FirebaseStorage.DeleteData(MovieDm(
                                    movie_id: "${popularResults.id}",
                                    doc_id: "${popularResults.id}",
                                    title: "${popularResults.title}",
                                    date: "${popularResults.releaseDate}",
                                    imagepath: "${popularResults.backdropPath}"
                                ));
                              } else {
